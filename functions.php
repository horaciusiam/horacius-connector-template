...
function is_client_allowed( $client_ip_address ) 
{
	$client_allowed = false;
	
	if ( in_array( $client_ip_address, $GLOBALS['allowed_clients'] ) )
	{
		$client_allowed = true;
	}
...
function decode_url_params( $data )
{
	...
	foreach ($params as $param ) 
	{
		...
		$decoded_params[ $values[0]  ] = $values[1];
	}
